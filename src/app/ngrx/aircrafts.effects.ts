...
import {
  ...
  SearchAircraftsActionError,
  SearchAircraftsActionSuccess,
...
export class AircraftsEffects {
  ...
  getAllAircraftsEffect: Observable<AircraftsActions> = createEffect(
    //nous souhaitons créer un effect ici sous condition, donc on écoute les actions
    () =>
      this.effectActions.pipe(
        //dès qu'une action arrive, on vérifie son type
        ...
        mergeMap((action: AircraftsActions) => {
          //mergeMap permet ici de renvoyer un Observable par action
          return this.aircraftService.getAircrafts().pipe(
            //attente réception des données en base (liste des avions)
